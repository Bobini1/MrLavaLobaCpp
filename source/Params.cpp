...
getDesc()
{
    ...
    desc.add_options()("help", "produce help message")("version",
                                                       "print version string")(
      "input_file",
      boost::program_options::value<std::string>(),
      "set input file");

    auto inputDataProperties =
      boost::program_options::options_description("Input data options");
    inputDataProperties.add_options()(
      ...
      "Fissure probabilities",
      boost::program_options::value<std::vector<int>>(),
      "this values defines the probabilities of the different segments of the "
      "polyline "
      "or of the different fissures.")(
      "hazard_flag",
      boost::program_options::value<bool>(),
      "Whether a raster map is saved where the values "
      "represent the probability of a cell to be covered. ")(
      "masking_threshold",
      boost::program_options::value<double>(),
      "Fraction of the volume emplaced or the area invaded (according to the "
      "flag "
      "flag_threshold) used to save the run_name_*_masked.asc files."
      "In this way we cut the \"tails\" with a low thickness "
      "(*_thickness_masked.asc) "
      "and a low probability (*_hazard_masked.asc). The file is used also "
      "to check the convergence of the solutions increasing the number of "
      "flows.\n"
      "The full outputs are saved in the files run_name_*_full.asc\n"
      "The masked files are saved only when masking_thresold < 1.")(
      "n_flows", boost::program_options::value<int>(), "Number of flows")(
      "min_n_lobes",
      ...
      "volume_flag",
      boost::program_options::value<bool>(),
      "total volume is read in input, and the "
      ...
      "total_volume",
      boost::program_options::value<double>(),
      "Total volume (this value is used only when volume_flag = true) set "
      "\"true\" to be confirmed.")(
      "fixed_dimension_flag",
      boost::program_options::value<bool>(),
      "This flag select which dimension of the lobe is fixed when "
      "volume_flag=true. "
      "If true, the thickness of the lobes is assigned. "
      "Otherwise, the area of the lobes is assigned")(
      "lobe_area",
      boost::program_options::value<double>(),
      "Area of each lobe ( only effective when volume_flag = false or "
      "fixed_dimension_flag = true )")(
      "thickness_ratio",
      boost::program_options::value<double>(),
      "Ratio between the thickness of the first lobe of the flow and the "
      "thickness of the "
      "last lobe.\n"
      ...
      "topo_mod_flag",
      boost::program_options::value<int>(),
      "This flag controls if the topography is modified by the lobes and if "
      "the "
      "emplacement of new flows is affected by the changed slope\n"
      "topo_mod_flag = 0   => the slope does not changes\n"
      "topo_mod_flag = 1   => the slope is re-evaluated every n_flows_counter "
      "flows\n"
      "topo_mod_flag = 2   => the slope is re-evaluated every n_lobes_counter "
      "flows"
      "and every n_flows_counter flows")(
      "n_flows_counter",
      boost::program_options::value<int>(),
      "This parameter is only effective when topo_mod_flag = 1 and defines the "
      "number of flows for the re-evaluation of the slope modified by the "
      "flow")(
      "n_lobes_counter",
      boost::program_options::value<int>(),
      "This parameter is only effective when topo_mod_flag = 2 and defines the"
      "number of lobes for the re-evaluation of the slope modified by the "
      "flow")(
      "thickening_parameter",
      boost::program_options::value<double>(),
      "This parameter (between 0 and 1) allows for a thickening of the flow "
      "giving "
      "controlling the modification of the slope due to the presence of the "
      "flow.\n"
      ...
      "if you reduce this, the impact of the lava flow is lessened in the "
      "computation of the slope, "
      "but the thickness is still correct. this allows for \"channel\" flow, "
      "if = 1, "
      ...
      "flag start_from_dist_flag=true) lobe. The closer is lobe_exponent to 0 "
      "the "
      ...
      boost::program_options::value<double>(),
      "Inertial exponent: \n"
      "inertial_exponent = 0 => the max probability direction for the new lobe "
      "is the "
      "max slope direction;\n"
      "inertial_exponent > 0 => the max probability direction for the new lobe "
      "takes "
    ...
    auto demCrop = boost::program_options::options_description(
    ...
    demCrop.add_options()(
      "east_to_vent",
      boost::program_options::value<double>(),
      "Distance from the vent to the eastern border of the DEM")(
      "west_to_vent",
      boost::program_options::value<double>(),
      "Distance from the vent to the western border of the DEM")(
      "north_to_vent",
      boost::program_options::value<double>(),
      "Distance from the vent to the northern border of the DEM")(
      "south_to_vent",
      boost::program_options::value<double>(),
      "Distance from the vent to the southern border of the DEM");

    auto ventCoords =
      boost::program_options::options_description("Coordinates of the vents");
    ventCoords.add_options()(
      "x_vent",
      boost::program_options::value<std::vector<int>>(),
      "Vent coordinates. Two values define a fissure instead.")(
      "y_vent",
      boost::program_options::value<std::vector<int>>(),
      "Vent coordinates. Two values define a fissure instead.");

    auto ventCoordsExtra = boost::program_options::options_description(
      "Extra coordinates of the vents. "
      "these coordinates are used when multiple fissures are defined:\n"
      "the first one goes from (x_vent[0],y_vent[0]) to "
      "(x_vent_end[0],y_vent_end[0])\n"
      "the second one goes from (x_vent[1],y_vent[1]) to "
      "(x_vent_end[1],y_vent_end[1])");
    ventCoordsExtra.add_options()(
      "x_vent_end",
      boost::program_options::value<std::vector<int>>(),
      "Vent end x coordinates.")(
      "y_vent_end",
      boost::program_options::value<std::vector<int>>(),
      "Vent end y coordinates.");

    ventCoords.add(ventCoordsExtra);

    auto advancedOptions =
      boost::program_options::options_description("Advanced options");
    advancedOptions.add_options()(
      "restart_files",
      boost::program_options::value<std::vector<std::string>>(),
      "this is a list of names of exiting *_thickness_*.asc files to be used "
      "if "
      ...
      "Example: restart_file = "
      "['flow_001_thickness_full.asc','flow_002_thickness_full.asc]")(
      "saveshape_flag",
      boost::program_options::value<bool>(),
      "If saveshape_flag = true then the output with the lobes is saved on a "
      "shapefile")("saveraster_flag",
                   boost::program_options::value<bool>(),
                   "If saveraster_flag = true then the raster output is saved "
                   "as a *.asc file")("severaster_flag",
                                      boost::program_options::value<bool>(),
                                      "If saveraster_flag = true then the "
                                      "raster output is saved as a *.asc file")(
      "plot_lobes_flag",
      boost::program_options::value<bool>(),
      ...
      "n_check_loop",
      boost::program_options::value<bool>(),
      "This parameter is to avoid that a chain of loop get stuck in a hole. It "
      "is "
      "active only when n_check_loop>0. When it is greater than zero the code "
      "check if the last n_check_loop lobes are all in a small box. If this is "
      "the "
      "case then the slope modified by flow is evaluated, and the hole is "
      "filled.")(
      ...
      "aspect_ratio_coeff",
      boost::program_options::value<double>(),
      "This parameter affect the shape of the lobes. The larger is this "
      "parameter, "
      "the larger is the effect of a small slope on the eccentricity of the "
      "lobes:\n"
      "aspect_ratio_coeff = 0  => the lobe is always a circle\n"
      "aspect_ratio_coeff > 0  => the lobe is an ellipse, with the aspect "
      "ratio "
      "increasing with the slope ")("max_aspect_ratio",
                                    boost::program_options::value<double>(),
    ...
    auto betaOptions = boost::program_options::options_description(
      "Beta options: The number of lobes of the flow is defined accordingly to "
      "a random uniform "
      "distribution or to a beta law, as a function of the flow number.\n"
      "a_beta, b_beta = 0  => n_lobes is sampled randomly in "
      "[min_n_lobes,max_n_lobes]\n"
      "a_beta, b_beta > 0  => n_lobes = min_n_lobes + 0.5 * ( max_n_lobes - "
      "min_n_lobes ) "
      "* beta(flow/n_flows,a_beta,b_beta)");
    betaOptions.add_options()("a_beta",
                              boost::program_options::value<double>(),
  ...
  : desc(getDesc())
{
    ...
    if (vm.contains("input_file")) {
        auto ifs = std::ifstream(vm["input_file"].as<std::string>());
        if (!ifs) {
            throw std::runtime_error("can not open config file " +
                                     vm["input_file"].as<std::string>());
        }
        boost::program_options::store(
          boost::program_options::parse_config_file(ifs, desc), vm);
        boost::program_options::notify(vm);
    }
}
auto
